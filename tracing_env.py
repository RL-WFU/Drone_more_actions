...
class Trace(Env):
    ...
    def reset_tracing(self, row, col):
        ...
        state = np.reshape(state, [1, 1, self.vision_size + (self.num_actions - 1)])
    ...
    def step(self, action, time):
        ...
        if action == 0: #up and left one grid
            if self.__class__.row_position > self.regions[self.__class__.current_target_index][0] \
                    and self.__class__.col_position > self.regions[self.__class__.current_target_index][1] \
            ...
            if self.__class__.row_position > self.sight_distance + 1 and self.__class__.row_position > self.regions[self.__class__.current_target_index][0]\
                    and self.__class__.col_position < (self.totalCols - self.sight_distance - 1) and self.__class__.col_position < self.regions[self.__class__.current_target_index][1] + 59:
                next_row = self.__class__.row_position - 1
                next_col = self.__class__.col_position + 1
            else:
                action = 4
            ...
            if self.__class__.col_position < (self.totalCols - self.sight_distance - 1) and self.__class__.col_position < self.regions[self.__class__.current_target_index][1] + 59:  # right one grid
                next_row = self.__class__.row_position
                next_col = self.__class__.col_position + 1
            else:
                action = 4
            ...
            if self.__class__.col_position > self.sight_distance + 1 and self.__class__.col_position > self.regions[self.__class__.current_target_index][1] \
                and self.__class__.row_position < (self.totalRows - self.sight_distance - 1) and self.__class__.row_position < self.regions[self.__class__.current_target_index][0] + 59:
                next_row = self.__class__.row_position + 1
                next_col = self.__class__.col_position - 1
            else:
                action = 4
        ...
        state = np.append(state, self.visited[self.__class__.row_position + 1, self.__class__.col_position + 1])
        # state = np.append(state, self.calculate_covered('region'))
        state = np.reshape(state, [1, 1, self.vision_size + 8])

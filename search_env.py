...
class Search(Env):
    ...
    def reset_search(self, row, col):
        ...
        state = np.reshape(state, [1, 1, self.vision_size + (self.num_actions - 1 + 2)])
    ...
    def step(self, action, time):
        ...
        if action == 0: #up and left one grid
            if self.__class__.row_position > self.local_map_lower_row and self.__class__.col_position > self.local_map_lower_col \
                    and self.__class__.row_position > self.sight_distance + 1 and self.__class__.col_position > self.sight_distance + 1:
                next_row = self.__class__.row_position - 1
                next_col = self.__class__.col_position - 1
            else:
                action = 9
        elif action == 1: #up one grid
            if self.__class__.row_position > self.local_map_lower_row and self.__class__.row_position > self.sight_distance + 1:
                next_row = self.__class__.row_position - 1
                next_col = self.__class__.col_position
            else:
                action = 9
        elif action == 2: #up and right one grid
            if self.__class__.row_position > self.local_map_lower_row and self.__class__.row_position > self.sight_distance + 1 \
                    and self.__class__.col_position < self.local_map_upper_col \
                    and self.__class__.col_position < (self.totalCols - self.sight_distance - 1):
                next_row = self.__class__.row_position - 1
                next_col = self.__class__.col_position + 1
            else:
                action = 9
        elif action == 3: #left one grid
            if self.__class__.col_position > self.local_map_lower_col and self.__class__.col_position > self.sight_distance + 1:
                next_row = self.__class__.row_position
                next_col = self.__class__.col_position - 1
            else:
                action = 9

        elif action == 5: #right one grid
        ...
        elif action == 6: #down and left one grid
            if self.__class__.col_position > self.local_map_lower_col and self.__class__.col_position > self.sight_distance + 1 \
                and self.__class__.row_position < self.local_map_upper_row and self.__class__.row_position < (self.totalRows - self.sight_distance - 1):
                next_row = self.__class__.row_position + 1
                next_col = self.__class__.col_position - 1
            else:
                action = 9
        elif action == 7: #down one grid
            if self.__class__.row_position < self.local_map_upper_row and self.__class__.row_position < (self.totalRows - self.sight_distance - 1):
                next_row = self.__class__.row_position + 1
                next_col = self.__class__.col_position
            else:
                action = 9
        elif action == 8: #down and right one grid
            if self.__class__.row_position < self.local_map_upper_row and self.__class__.row_position < (self.totalRows - self.sight_distance - 1) \
                and self.__class__.col_position < self.local_map_upper_col and self.__class__.col_position < (
                    self.totalCols - self.sight_distance - 1):
                next_row = self.__class__.row_position + 1
                next_col = self.__class__.col_position + 1
            else:
                action = 9

        elif action == 4 or action == 9:  # This hardcodes the drone to move towards the target if it tries to take an invalid action
            #could optimize this to check for target location first if too slow
            if self.__class__.row_position > self.local_map_lower_row and self.__class__.col_position > self.local_map_lower_col \
                    and self.__class__.row_position > self.sight_distance + 1 and self.__class__.col_position > self.sight_distance + 1 \
                    and self.__class__.row_position > self.local_target[0] and self.__class__.col_position > self.local_target[1]:
                next_row = self.__class__.row_position - 1
                next_col = self.__class__.col_position - 1
            elif self.__class__.row_position > self.local_map_lower_row and self.__class__.row_position > self.sight_distance + 1 \
                    and self.__class__.row_position > self.local_target[0] and self.__class__.col_position == self.local_target[1]:
                next_row = self.__class__.row_position - 1
                next_col = self.__class__.col_position
            elif self.__class__.row_position > self.local_map_lower_row and self.__class__.row_position > self.sight_distance + 1 \
                ...
                next_row = self.__class__.row_position - 1
                next_col = self.__class__.col_position + 1
            elif self.__class__.col_position > self.local_map_lower_col and self.__class__.col_position > self.sight_distance + 1 \
                    and self.__class__.col_position > self.local_target[1] and self.__class__.row_position == self.local_target[0]:
                next_row = self.__class__.row_position
                next_col = self.__class__.col_position - 1
            elif self.__class__.col_position < self.local_map_upper_col and self.__class__.col_position < (self.totalCols - self.sight_distance - 1) \
                    and self.__class__.col_position < self.local_target[1] and self.__class__.row_position == self.local_target[0]:
                next_row = self.__class__.row_position
                next_col = self.__class__.col_position + 1
            elif self.__class__.col_position > self.local_map_lower_col and self.__class__.col_position > self.sight_distance + 1 \
                ...
                next_row = self.__class__.row_position + 1
                next_col = self.__class__.col_position - 1
            elif self.__class__.row_position < self.local_map_upper_row and self.__class__.row_position < (self.totalRows - self.sight_distance - 1) \
                    and self.__class__.row_position < self.local_target[0] and self.__class__.col_position == self.local_target[1]:
                next_row = self.__class__.row_position + 1
                next_col = self.__class__.col_position
            elif self.__class__.row_position < self.local_map_upper_row and self.__class__.row_position < (self.totalRows - self.sight_distance - 1) \
        ...
        state = np.append(state, self.visited[self.__class__.row_position - 1, self.__class__.col_position - 1])
    ...
    def step_local(self, action, time):
        ...
        if action == 0:  # up and left one grid
            ...
            if self.__class__.col_position > self.local_map_lower_col and self.__class__.col_position > self.sight_distance + 1 \
                    and self.__class__.row_position < self.local_map_upper_row and self.__class__.row_position < (
                    self.totalRows - self.sight_distance - 1):
                next_row = self.__class__.row_position + 1
                next_col = self.__class__.col_position - 1
            else:
                action = 9
            ...
            if self.__class__.row_position > self.local_map_lower_row and self.__class__.col_position > self.local_map_lower_col \
                    and self.__class__.row_position > self.sight_distance + 1 and self.__class__.col_position > self.sight_distance + 1 \
                    and self.__class__.row_position > self.local_target[0] and self.__class__.col_position > \
                    self.local_target[1]:
                next_row = self.__class__.row_position - 1
                next_col = self.__class__.col_position - 1
            elif self.__class__.row_position > self.local_map_lower_row and self.__class__.row_position > self.sight_distance + 1 \
                ...
                next_col = self.__class__.col_position + 1
                    ...
                    and self.__class__.col_position > self.local_target[1] and self.__class__.row_position == \
                    self.local_target[0]:
                next_row = self.__class__.row_position
                next_col = self.__class__.col_position - 1
                ...
                next_row = self.__class__.row_position + 1
                next_col = self.__class__.col_position - 1
            elif self.__class__.row_position < self.local_map_upper_row and self.__class__.row_position < (
                    self.totalRows - self.sight_distance - 1) \
                    and self.__class__.row_position < self.local_target[0] and self.__class__.col_position == \
                    self.local_target[1]:
                next_row = self.__class__.row_position + 1
